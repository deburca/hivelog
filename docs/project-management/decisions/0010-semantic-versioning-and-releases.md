...
a `releases/` area with per-version notes (`releases/v1.1.0.md`). There is no
written rule for what bumps which version component or what a release must
satisfy.
...
fixes are **patch**. Each release has a `releases/vX.Y.Z.md` note + checklist, is
cut only when the `--group hivelog` suite is green and `drush updb` is clean, and
bumps `hivelog.info.yml`.
