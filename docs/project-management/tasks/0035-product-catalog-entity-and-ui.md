...
## Acceptance criteria
- [ ] `src/Entity/Product.php` — `ContentEntityBase` with
      `#[ContentEntityType]`, base table `hivelog_product`, entity keys
      (`id`, `label` → `name`, `uuid`, `owner` → `uid`).
- [ ] Fields: `apiary` (required entity_reference → `apiary`), `name`
...
- [ ] `ProductAccessControlHandler` using `ApiaryAccessTrait::resolveApiary()`
...
- [ ] Standard entity CRUD routes (`entity.product.*`) plus an
      apiary-scoped add route (`hivelog.product.add`), mirroring
      `hivelog.inventory_item.add`.
- [ ] `ProductListBuilder` — self-built "Add Product" heading using the
...
- [ ] `ProductController` — sectioned canonical `view()` page following
      `InventoryItemController`'s buildActions/buildSection/buildRows/
      buildFieldValue shape.
- [ ] Apiary canonical page: a "Products" heading + embedded,
      ...
      `INVENTORY_ITEMS_PAGER_ELEMENT`), `view`-before-`add` button
      ordering in the heading.
- [ ] `hivelog.permissions.yml`: `view own product`, `view any product`,
      `add product`, `edit own product`, `edit any product`, `delete own
      product`, `delete any product`.
- [ ] `hivelog_update_NNNN` in `hivelog.install` installs the new entity
      type; add `product` to `hivelog_uninstall()`'s cleanup list before
      `apiary`.
- [ ] Kernel tests (`ProductTest`, `ProductAccessTest`): CRUD, field
