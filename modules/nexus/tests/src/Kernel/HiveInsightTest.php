 ...
 *
 * Relocated here, unchanged in substance, from `collective` by task
 * 0102 — see [[0100-nexus-in-process-ai-synthesis]].
 */
#[Group('hivelog')]
...
class HiveInsightTest extends KernelTestBase {
   ...
   *
   * Also the one place that verifies
   * `\Drupal\hivelog\ApiaryAccessTrait::resolveApiary()`'s `hive_insight`
   * branch (added to core in task 0089, dormant while the entity lived
   * only in `collective`'s test suite) still resolves correctly now that
   * `HiveInsight` is defined by `nexus` instead — the branch only checks
   * `getEntityTypeId()`, so no core change was expected or needed.
   */
