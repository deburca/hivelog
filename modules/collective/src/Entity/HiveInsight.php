...
use Drupal\collective\HiveInsightAccessControlHandler;
 ...
 * clear." Machine-written only, by `InsightAgent`'s own write-back
 * request — no add/edit form is ever built for this entity type, same
 * "machine-written only" shape as `nanoprobe`'s `SensorReading`.
 *
 * See docs/project-management/decisions/0088-ai-insights-implementation.md
 * §1 for the full schema this implements.
 */
#[ContentEntityType(
  id: 'hive_insight',
  ...
  base_table: 'collective_hive_insight',
...
class HiveInsight extends ContentEntityBase implements EntityChangedInterface {
  ...
  public static function baseFieldDefinitions(EntityTypeInterface $entity_type) {
    ...
    $fields['confidence'] = BaseFieldDefinition::create('list_string')
      ...
      ->setDescription(t('How confident the agent is in this verdict, if it reports one.'))
    ...
    $fields['context_snapshot'] = BaseFieldDefinition::create('string_long')
      ...
      ->setDescription(t('A copy of what the context-read endpoint returned for this hive/apiary at generation time — an audit trail for what the agent actually saw.'));

    $fields['generated'] = BaseFieldDefinition::create('timestamp')
      ->setLabel(t('Generated'))
      ->setDescription(t('When the agent produced this insight — supplied by the agent, not HiveLog\'s own receipt time.'))
      ->setRequired(TRUE)
      ->setDisplayOptions('view', [
        'label' => 'inline',
        'type' => 'timestamp',
    ...
    $fields['created'] = BaseFieldDefinition::create('created')
      ->setLabel(t('Created'))
      ->setDescription(t('The time HiveLog received this insight.'));
